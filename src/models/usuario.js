...
static async actualizar(id, datos) {
    ...
    // Usamos UPDATE (actualizar) y WHERE (donde/que)
    const query = 'UPDATE users SET nombre = ?, email = ?, rol = ? WHERE id = ?';
    
    try {
        const resultado = await executeQuery(query, [nombre, email, rol, id]);
        
        // "affectedRows" nos dice si realmente se encontró el ID y se actualizó
...
static async eliminar(id){
    ...
    try{
        const respuesta = await executeQuery(query, [id]);
        return resultado.affectedRows > 0;
    }catch(error){
